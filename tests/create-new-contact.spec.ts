...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Enter your email, phone, or' }).fill('emeraldchen@emeraldchen25.onmicrosoft.com');
  await page.getByRole('textbox', { name: 'Enter your email, phone, or' }).click();
  await page.getByRole('button', { name: 'Next' }).click();
  await page.locator('#i0118').fill('*WRNf2025');
...
await page.getByRole('textbox', { name: 'First Name' }).fill('Bartolomea');
...
await expect(page.getByRole('link', { name: 'Bartolomea Thomas' })).toBeVisible();
});